...
class App:
    def __init__(self,root):
        self.root=root
        self.root.title('Bass MIDI GUI PC v8')
    ...
    def build_top(self):
        ...
        ttk.Button(nav,text='← 前の8小節',command=lambda:self.move_page(-1)).pack(side='left',padx=3)
        ttk.Button(nav,text='次の8小節 →',command=lambda:self.move_page(1)).pack(side='left',padx=3)
    ...
    def move_page(self,delta):
        max_page=max(0,(len(self.data)-1)//VISIBLE_BARS)
    ...
    def batch_key_apply(self):
        ...
        end=min(start+VISIBLE_BARS,len(self.data))
    ...
    def render(self):
        ...
        for row,i in enumerate(range(start,end)):
            self.build_bar(container,i,self.data[i],row)

    def build_bar(self,parent,index,b,row):
        ...
        def make_chord_input(parent_row,label,var,oct_var):
            ...
            box.pack(side='left')
            ttk.Spinbox(parent_row,from_=0,to=8,textvariable=oct_var,width=4).pack(side='left',padx=3)
            return box
        ...
        for i in range(count):
            ...
            ttk.Entry(cell,textvariable=sv,width=7).pack()
