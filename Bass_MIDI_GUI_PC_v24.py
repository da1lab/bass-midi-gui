...
class App:
    def __init__(self,root):
        self.root=root
        self.root.title('Bass MIDI GUI PC v24')
    ...
    def bar_to_dict(self,b):
        return {'key':b.key,'split':b.split,'chord':b.chord,'octave':b.octave,
    ...
    def dict_to_bar(self,item):
        b=Bar(); b.key=item.get('key','C') or 'C'; b.split=bool(item.get('split',False)); b.chord=item.get('chord','')
        ...
        b.bass=list(item.get('bass',[])); return b
    ...
    def apply_state_dict(self,state):
        ...
        for i,t in enumerate(src_tracks):
            bars=[self.dict_to_bar(x) for x in t.get('bars',[])] or [Bar()]
    ...
    def load_project(self):
        ...
        try:
            ...
            messagebox.showinfo('プロジェクト読込','プロジェクトを読み込みました。')
        except Exception as e:
            messagebox.showerror('プロジェクト読込エラー',str(e))
