...
class App:
    def __init__(self,root):
        self.root=root
        self.root.title('Bass MIDI GUI PC v15')
    ...
    def build_bar(self,parent,index,b,row,col):
        ...
        key_box=ttk.Combobox(key_row,textvariable=key,values=ROOTS,state='readonly',width=4,takefocus=False)
        ...
        def make_chord_input(parent_row,label,var,oct_var):
            ...
            box=ttk.Combobox(parent_row,textvariable=var,values=CHORDS,width=8,height=18,takefocus=True)
            box.pack(side='left',padx=(0,1))
            tk.Button(parent_row,text='#/b',width=3,padx=1,pady=1,font=('',12,'bold'),takefocus=0,command=lambda v=var:self.toggle_chord_var(v)).pack(side='left',padx=(1,2))
        ...
        for i in range(count):
            ...
            entry=ttk.Entry(cell,textvariable=sv,width=4,justify='center',takefocus=True)
            entry.pack()
            tk.Button(cell,text='#/b',width=3,padx=1,pady=0,font=('',11,'bold'),takefocus=0,command=lambda v=sv:self.toggle_note_var(v)).pack(pady=(1,1))
