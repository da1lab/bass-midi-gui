...
class App:
    def __init__(self,root):
        self.root=root
        self.root.title('Bass MIDI GUI PC v26')
        self.root.geometry('1500x900')
        self.root.minsize(1100,650)
        self.data=[Bar() for _ in range(DEFAULT_BARS)]
        self.tracks=[{'no':1,'name':'Track 01','bpm':DEFAULT_BPM,'resolution':'4分音符','auto_low':'G2','auto_high':'F#3','bars':self.data}]
    ...
    def build_top(self):
        ...
        self.track_name_var=tk.StringVar(value='Track 01')
        ...
        chord_box=ttk.Combobox(top,textvariable=self.batch_chord_var,values=CHORDS,width=10,state='readonly')
    ...
    def open_file_menu(self):
        ...
        ttk.Button(body,text='MIDI保存',width=18,command=self.create_midi).grid(row=5,column=0,padx=4,pady=4,sticky='ew')
    ...
    def open_track_config(self):
        """Track名の確認・編集、入力済小節数の参照、Track追加を行う。"""
        ...
        ttk.Label(outer,text='Track',font=('',11,'bold')).grid(row=0,column=0,padx=5,pady=5)
        ttk.Label(outer,text='トラック名',font=('',11,'bold')).grid(row=0,column=1,padx=5,pady=5)
        ttk.Label(outer,text='入力済小節数',font=('',11,'bold')).grid(row=0,column=2,padx=5,pady=5)
        rows=ttk.Frame(outer)
        rows.grid(row=1,column=0,columnspan=3,sticky='nsew')
        ...
        def rebuild():
            ...
            for r,t in enumerate(self.tracks):
                ttk.Label(rows,text=f"{int(t.get('no',r+1)):02d}",width=8,anchor='center').grid(row=r,column=0,padx=5,pady=5)
                v=tk.StringVar(value=t.get('name') or f"Track {r+1:02d}")
                name_vars.append(v)
                ttk.Entry(rows,textvariable=v,width=32).grid(row=r,column=1,padx=5,pady=5,sticky='ew')
                ttk.Label(rows,text=str(self.track_input_bar_count(t)),width=12,anchor='center').grid(row=r,column=2,padx=5,pady=5)
            rows.columnconfigure(1,weight=1)
        def apply_names():
            for i,v in enumerate(name_vars):
                if i<len(self.tracks):
                    no=int(self.tracks[i].get('no',i+1))
                    self.tracks[i]['name']=v.get().strip() or f'Track {no:02d}'
        ...
        ttk.Button(buttons,text='+ Track追加',command=add_from_config).pack(side='left',padx=4)
        ttk.Button(buttons,text='適用',command=apply_names).pack(side='right',padx=4)
        ttk.Button(buttons,text='閉じる',command=lambda:(apply_names(),win.destroy())).pack(side='right',padx=4)
        win.protocol('WM_DELETE_WINDOW',lambda:(apply_names(),win.destroy()))
        win.focus_set()
    ...
    def add_track(self):
        ...
        self.tracks.append({'no':no,'name':f'Track {no:02d}','bpm':DEFAULT_BPM,'resolution':'4分音符','auto_low':'G2','auto_high':'F#3','bars':bars})
    ...
    def apply_state_dict(self,state):
        ...
        if state.get('format')=='bass-midi-gui-multitrack' or state.get('tracks'):
            ...
            src_tracks=[{'no':1,'name':'Track 01','bpm':state.get('bpm',DEFAULT_BPM),'resolution':state.get('resolution','4分音符'),
        ...
        for i,t in enumerate(src_tracks):
            ...
            tracks.append({'no':int(t.get('no',i+1)),'name':t.get('name') or f'Track {i+1:02d}','bpm':int(round(float(t.get('bpm',DEFAULT_BPM)))),
    ...
    def load_project(self):
        ...
        try:
            ...
            self.root.title(f'Bass MIDI GUI PC v26 - {Path(path).name}')
            print(f'プロジェクト読込完了: {path}')
        except Exception as e:
            # エラー時もNSAlertを避け、Python自体が落ちないようにする。
            print(f'プロジェクト読込エラー: {type(e).__name__}: {e}')
            self.root.title(f'Bass MIDI GUI PC v26 - 読込エラー')
