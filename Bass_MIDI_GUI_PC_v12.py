...
class App:
    def __init__(self,root):
        self.root=root
        self.root.title('Bass MIDI GUI PC v12')
    ...
    def build_bar(self,parent,index,b,row,col):
        ...
        def make_chord_input(parent_row,label,var,oct_var):
            ...
            tk.Button(parent_row,text='#/b',width=2,padx=0,pady=0,font=('',7),command=lambda v=var:self.toggle_chord_var(v)).pack(side='left',padx=(0,1))
        ...
        if not b.split:
            ...
            chord_degree_label=ttk.Label(chord_row,text='',foreground='blue',width=9)
            ...
            chord_degree_label=ttk.Label(chord_row,text='',foreground='blue',width=11)
            chord_degree_label.pack(side='left',padx=3)

        ttk.Checkbutton(key_row,text='2拍分割',variable=split).pack(side='left',padx=3)

        bass_row=ttk.Frame(frame)
        bass_row.grid(row=2,column=0,sticky='w',padx=4,pady=(2,4))
        ttk.Label(bass_row,text='Bass',font=('',8)).grid(row=0,column=0,padx=(0,3),sticky='nw')
        ...
        for i in range(count):
            ...
            cell.grid(row=beat,column=subdivision+1,padx=1,pady=1,sticky='n')
            sv=tk.StringVar(value=b.bass[i])
            if subdivision==0:
                ttk.Label(bass_row,text=f'{beat+1}',font=('',8)).grid(row=beat,column=0,padx=(0,3),sticky='e')
            ttk.Label(cell,text=f'{subdivision+1}',font=('',7)).pack()
            note_row=ttk.Frame(cell)
            note_row.pack()
            entry=ttk.Entry(note_row,textvariable=sv,width=5)
            entry.pack(side='left')
            tk.Button(note_row,text='#/b',width=2,padx=0,pady=0,font=('',6),command=lambda v=sv:self.toggle_note_var(v)).pack(side='left',padx=(1,0))
            ...
            degree_frame.pack()
            dg_key=ttk.Label(degree_frame,text='',foreground='green',width=3,font=('',7))
            dg_key.pack(side='left')
            dg_chord=ttk.Label(degree_frame,text='',foreground='purple',width=5,font=('',7))
