...
class App:
    def __init__(self,root):
        self.root=root
        self.root.title('Bass MIDI GUI PC v25')
    ...
    def build_top(self):
        ...
        ttk.Button(top,text='+Track',command=self.add_track).pack(side='left',padx=(0,3))
        ttk.Label(top,text='名前').pack(side='left')
        self.track_name_var=tk.StringVar(value='Track 01')
        name_entry=ttk.Entry(top,textvariable=self.track_name_var,width=12)
        name_entry.pack(side='left',padx=(3,10))
        name_entry.bind('<FocusOut>',lambda e:self.sync_track_settings())
        ...
        ttk.Button(nav,text='プロジェクト保存',command=self.save_project).pack(side='left',padx=(10,3))
        ttk.Button(nav,text='プロジェクト読込',command=self.load_project).pack(side='left',padx=3)
    ...
    def render(self):
        ...
        ttk.Button(container,text='#/♭',width=5,command=lambda i=i:self.toggle_bar_accidental(i)).grid(row=0,column=99,padx=5,pady=4,sticky='e')
    ...
    def dict_to_bar(self,item):
        ...
        b.accidental=item.get('accidental','sharp') or 'C'; b.split=bool(item.get('split',False)); b.chord=item.get('chord','')
    ...
    def state_dict(self):
        ...
        return {'format':'bass-midi-gui-multitrack','format_version':3,'app':'Bass MIDI GUI PC','version':'24',
    ...
    def load_project(self):
        ...
        try:
            ...
            self.root.title(f'Bass MIDI GUI PC v25 - {Path(path).name}')
            print(f'プロジェクト読込完了: {path}')
        except Exception as e:
            # エラー時もNSAlertを避け、Python自体が落ちないようにする。
            print(f'プロジェクト読込エラー: {type(e).__name__}: {e}')
            self.root.title(f'Bass MIDI GUI PC v25 - 読込エラー')
