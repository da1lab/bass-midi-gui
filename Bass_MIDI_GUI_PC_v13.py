...
CHORDS = [r+t for r in ROOTS_ALL for t in CHORD_TYPES]
...
def toggle_chord_spelling(chord):
    ...
    if not chord:
        return chord
    normalized=chord[0].upper()+chord[1:]
...
class App:
    def __init__(self,root):
        self.root=root
        self.root.title('Bass MIDI GUI PC v13')
    ...
    def build_bar(self,parent,index,b,row,col):
        ...
        key_box=ttk.Combobox(key_row,textvariable=key,values=ROOTS,state='readonly',width=4)
        ...
        def make_chord_input(parent_row,label,var,oct_var):
            ttk.Label(parent_row,text=label).pack(side='left',padx=(3,1))
            box=ttk.Combobox(parent_row,textvariable=var,values=CHORDS,width=8,height=18)
            box.pack(side='left',padx=(0,1))
            tk.Button(parent_row,text='#/b',width=3,padx=1,pady=1,font=('',10,'bold'),command=lambda v=var:self.toggle_chord_var(v)).pack(side='left',padx=(1,2))
            ttk.Spinbox(parent_row,from_=0,to=8,textvariable=oct_var,width=3).pack(side='left',padx=(0,2))
            return box

        if not b.split:
            make_chord_input(chord_row,'',chord,chord_oct)
            chord_degree_label=ttk.Label(chord_row,text='',foreground='blue',width=9,font=('',10,'bold'))
            ...
            chord_degree_label=ttk.Label(chord_row,text='',foreground='blue',width=11,font=('',10,'bold'))
            chord_degree_label.pack(side='left',padx=3)

        ttk.Checkbutton(key_row,text='2拍分割',variable=split).pack(side='left',padx=3)

        bass_row=ttk.Frame(frame)
        bass_row.grid(row=2,column=0,sticky='w',padx=4,pady=(3,5))
        ttk.Label(bass_row,text='Bass',font=('',9,'bold')).grid(row=0,column=0,padx=(0,3),sticky='nw')
        ...
        for i in range(count):
            ...
            ttk.Label(cell,text=pos_text,font=('',8)).pack(pady=(0,1))
            entry=ttk.Entry(cell,textvariable=sv,width=4,justify='center')
            entry.pack()
            tk.Button(cell,text='#/b',width=3,padx=1,pady=0,font=('',9,'bold'),command=lambda v=sv:self.toggle_note_var(v)).pack(pady=(1,1))
            dg_key=ttk.Label(cell,text='',foreground='green',width=5,anchor='center',font=('',10,'bold'))
            dg_key.pack()
            dg_chord=ttk.Label(cell,text='',foreground='purple',width=6,anchor='center',font=('',10,'bold'))
            dg_chord.pack()
        ...
        legend.grid(row=3,column=0,sticky='w',padx=5,pady=(0,3))
        tk.Label(legend,text='● Keyから見た度数',fg='green',font=('',10,'bold')).pack(side='left',padx=(0,10))
        tk.Label(legend,text='● コードルートから見た度数',fg='purple',font=('',10,'bold')).pack(side='left')
