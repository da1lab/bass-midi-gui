...
class App:
    def __init__(self,root):
        self.root=root
        self.root.title('Bass MIDI GUI PC v21')
    ...
    def state_dict(self):
        self.commit_current_view()
        return {
            'app':'Bass MIDI GUI PC','version':21,
    ...
    def save_project(self):
        try:
            ...
            state['format']='bassproj'
            state['format_version']=1
            path=filedialog.asksaveasfilename(
                defaultextension='.bassproj',
                filetypes=[('Bass MIDI GUI Project','*.bassproj'),('JSON','*.json'),('すべてのファイル','*.*')]
            )
            if not path:
                return
            Path(path).write_text(json.dumps(state,ensure_ascii=False,indent=2),encoding='utf-8')
            messagebox.showinfo('プロジェクト保存','PC/iPhone共通プロジェクトを保存しました。')
    ...
    def load_project(self):
        ...
        try:
            ...
            if state.get('format') not in (None,'bassproj'):
